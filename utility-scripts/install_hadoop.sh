...
download__hadoop () {
	...
	echo 'export HADOOP_INSTALL=/home/user/hadoop-3.2.2' >> ~/.bashrc
...
## Edit core-site.xml to set hdfs default path to hdfs://master:9000
		...
		sed -i "/<\/configuration>/ s/.*/${INPUT_CORE_SITE_CONTENT}\n&/" /home/user/hadoop-3.2.2/etc/hadoop/core-site.xml
...
## Set the two datanodes for the distributed filesystem
		echo "master" > /home/user/hadoop-3.2.2/etc/hadoop/slaves
		echo "slave1" >> /home/user/hadoop-3.2.2/etc/hadoop/slaves
		echo "slave2" >> /home/user/hadoop-3.2.2/etc/hadoop/slaves

## Export JAVA_HOME variable for hadoop
		sed -i '/export JAVA\_HOME/c\export JAVA\_HOME=\/usr\/lib\/jvm\/java-8-openjdk-amd64' /home/user/hadoop-3.2.2/etc/hadoop/hadoop-env.sh
}
